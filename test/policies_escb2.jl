@testset "ESCB2" begin
  ...
  @testset "Budgeted (one by one)" begin
    ...
    m = 2
    r = Distribution[Bernoulli(.5 + ((i % 2 == 0) ? ε : 0.)) for i in 1:n]
    i = MSet(r, m, MSetLPSolver(Cbc.Optimizer))
    ...
    @testset "Trace" begin
      n_rounds = 20
      test_policy_trace(i, ESCB2(ESCB2Budgeted(.1, false)), n_rounds, n_rounds * m, 1.2, 21.0, 22.2)
    end
  end
  ...
  @testset "optimise_linear_sqrtlinear" begin
    ...
    ilp = MSet(distr, m, MSetLPSolver(solver))
